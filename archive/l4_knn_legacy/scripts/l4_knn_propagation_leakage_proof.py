...
def load_golden_labels(golden_dir: Path) -> dict:
    """加载黄金集人工标注（高置信种子）
    支持两种目录结构：
      - golden_dir/annotations/*.json（旧格式）
      - golden_dir/*.json（新格式，Qwen-Omni直接输出）
    支持两种字段名：
      - annotation.genre_level1（旧格式）
      - annotation.genre（新格式）
    """
    labels = {}
    # 收集所有JSON文件：优先annotations子目录，其次根目录
    json_files = []
    ann_dir = golden_dir / "annotations"
    if ann_dir.exists():
        json_files.extend(list(ann_dir.glob("*.json")))
    # 根目录的JSON（排除raw子目录）
    ...
    for f in json_files:
        ...
        if ulid:
            ...
            label_dict = {}
            # 支持 genre_level1（旧）和 genre（新）
            genre = ann.get("genre_level1") or ann.get("genre")
            if genre:
                label_dict["genre"] = genre
            ...
            if ann.get("instruments") and len(ann["instruments"]) > 0:
                # instruments可能是字符串列表或对象列表
                instr = ann["instruments"][0]
                if isinstance(instr, dict):
                    instr = instr.get("name", str(instr))
                label_dict["instruments"] = str(instr)
