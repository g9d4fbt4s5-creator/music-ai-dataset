...
# "使用中"判定（任一命中即重置闲置计数）：
#   1. GPU 利用率 > 5%
#   2. 存在 labelstudio-env 环境跑的 python 任务进程
#      （排除 jupyter/tensorboard/本看门狗自身）
#   3. 有活跃交互会话（who 输出非空，detached screen 不算）
#
# 部署（SSH 进实例后）：
#   screen -dmS watchdog bash /root/idle_watchdog.sh
# 查看日志：
...
while true; do
  ...
  # --- 2. 任务进程：labelstudio-env 的 python（排除常驻服务与本脚本） ---
  task_n=$(ps aux | grep '[l]abelstudio-env/bin/python' | grep -vE 'jupyter|tensorboard|idle_watchdog' | wc -l | tr -d '[:space:]')
...
# ===== 开机自启（可选）=====
# AutoDL 容器重启后 screen 会话丢失。若希望开机自动拉起看门狗，
# 可将本脚本复制到 /root/autodl-tmp/ 后，在 ~/.bashrc 末尾加入：
#
#   if [ -z "$IDLE_WD_STARTED" ] && [ -f /root/autodl-tmp/idle_watchdog.sh ]; then
#     export IDLE_WD_STARTED=1
#     screen -dmS watchdog bash /root/autodl-tmp/idle_watchdog.sh
#   fi
#
# 注意：.bashrc 仅交互登录触发；非交互开机不会执行。
# 真正的开机自启需借助 AutoDL 创建实例时的 start_command（API）或控制台启动命令。
